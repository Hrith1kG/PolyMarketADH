...
def build(asset_slug="bitcoin-up-or-down-r1", seconds_left=20, market_id="900",
          up=0.95, down=0.05, tokens=("r1-up", "r1-down")):
    market = make_market(slug=asset_slug, seconds_left=seconds_left, market_id=market_id,
...
configure()
broker = fresh_broker()
market, client = build(asset_slug="ethereum-up-or-down-r2", market_id="901",
...
later_market = make_market(slug="ethereum-up-or-down-r2", seconds_left=20 + 300,
...
for window in (30, 60):
    ...
    broker = fresh_broker()
    market, client = build(asset_slug="solana-up-or-down-w", market_id=f"91{window}",
    ...
    market2, client2 = build(asset_slug="solana-up-or-down-e", market_id=f"92{window}",
...
configure()
broker = fresh_broker()
market, client = build(asset_slug="xrp-up-or-down-r3", market_id="930", seconds_left=2,
...
configure()
broker = fresh_broker()
market, client = build(asset_slug="dogecoin-up-or-down-r4", market_id="940",
...
broker = fresh_broker()
market, client = build(asset_slug="bitcoin-up-or-down-r5", market_id="950",
...
for key, value in [("crypto_entry_kill_switch", True), ("crypto_bot_status", "PAUSED")]:
    configure(**{key: value})
    broker = fresh_broker()
    market, client = build(asset_slug="bitcoin-up-or-down-r6", market_id="960",
...
broker = fresh_broker()
m1, c1 = build(asset_slug="bitcoin-up-or-down-r7", market_id="970", tokens=("r7-up", "r7-down"))
s1, _ = strategy(broker, c1)
assert s1.run_once(now=NOW).entries == 1
m2, c2 = build(asset_slug="ethereum-up-or-down-r8", market_id="971", tokens=("r8-up", "r8-down"))
...
broker = fresh_broker()
m1, c1 = build(asset_slug="bitcoin-up-or-down-r9", market_id="980", tokens=("r9-up", "r9-down"))
s1, _ = strategy(broker, c1)
assert s1.run_once(now=NOW).entries == 1
assert broker.crypto_trades_today() == 1
m2, c2 = build(asset_slug="solana-up-or-down-r10", market_id="981", tokens=("r10-up", "r10-down"))
s2, _ = strategy(broker, c2)
...
broker = fresh_broker()
market, client = build(asset_slug="bitcoin-up-or-down-r11", market_id="990", tokens=("r11-up", "r11-down"))
strat, _ = strategy(broker, client)
result = strat.run_once(now=NOW)
...
broker = fresh_broker()
noise = [
    make_market(slug="bitcoin-up-or-down-far", seconds_left=200, market_id="1100", tokens=("f-up", "f-down")),
    make_market(slug="ethereum-up-or-down-cheap", seconds_left=10, market_id="1101", tokens=("c-up", "c-down"), prices=(0.60, 0.40)),
]
...
def live_run(outcome, slug, market_id, tokens):
    configure(live_trading=True)
    broker = fresh_broker()
    _market, client = build(asset_slug=slug, market_id=market_id, tokens=tokens)
...
broker, live, result = live_run("filled", "bitcoin-up-or-down-L1", "1200", ("L1-up", "L1-down"))
...
broker, live, result = live_run("resting", "ethereum-up-or-down-L2", "1201", ("L2-up", "L2-down"))
assert result.entries == 1, result
...
broker, live, result = live_run("rejected", "solana-up-or-down-L3", "1202", ("L3-up", "L3-down"))
assert result.entries == 0, result
...
configure(live_trading=True)
broker = fresh_broker()
_market, client = build(asset_slug="xrp-up-or-down-L4", market_id="1203", tokens=("L4-up", "L4-down"))
